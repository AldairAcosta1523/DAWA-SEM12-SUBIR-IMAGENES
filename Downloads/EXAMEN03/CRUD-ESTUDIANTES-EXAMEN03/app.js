...
app.get('/delete/:id', async (req, res) => {
  ...
  try {
    const [row] = await connection.promise().query('SELECT imagen FROM estudiantes WHERE id = ?', [id]);
...
app.post('/save', upload.single('imagen'), async (req, res) => {
  ...
  try {
    ...
    await connection.promise().query(
      ...
      { nombre, apellidos, seccion, correo, edad, imagen: s3Key }
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/edit/:id', async (req, res) => {
  ...
  try {
    ...
    if (estudiantes && estudiantes.imagen) {
      const imageUrl = `/images/${estudiantes.imagen}`; // URL de la imagen
      res.render('edit', { estudiantes: estudiantes, imageUrl: imageUrl });
    } else {
...
app.post('/update', upload.single('imagen'), async (req, res) => {
  ...
  try {
    await connection.promise().query(
      'UPDATE estudiantes SET nombre = ?, apellidos = ?, seccion = ?, correo = ?, edad = ?, imagen = ? WHERE id = ?',
      [nombre, apellidos, seccion, correo, edad, imagen, id]
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/delete-all', async (req, res) => {
  try {
    const [rows] = await connection.promise().query('SELECT imagen FROM estudiantes');
